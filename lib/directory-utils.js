...
export function formatDirectoryInfo(analysis) {
  ...
  if (analysis.buildArtifacts && analysis.buildArtifacts.length > 0) {
...
export function performSafetyChecks(analysis) {
  ...
  // Handle case where directory doesn't exist or analysis failed
  ...
  if (analysis.hasPackageJson) {
    blocks.push({
      ...
      details: `Found package.json with ${analysis.configFiles ? analysis.configFiles.length : 0} config files`,
      solutions: [
  ...
  if (analysis.buildArtifacts && analysis.buildArtifacts.length > 0) {
    blocks.push({
      ...
      message: `Directory contains build artifacts: ${analysis.buildArtifacts ? analysis.buildArtifacts.join(', ') : 'unknown'}`,
