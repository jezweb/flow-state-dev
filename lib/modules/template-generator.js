...
const { readFile, writeFile, ensureDir, pathExists, copy } = fsExtra;
...
export class TemplateGenerator {
  ...
  registerHelpers() {
    ...
    // Missing helpers from templates
    Handlebars.registerHelper('isSignUp', function(options) {
      // Check if context has signup flag
      return this.isSignUp ? options.fn(this) : options.inverse(this);
    });
    
    Handlebars.registerHelper('loading', function(options) {
      // Check if context has loading flag
      return this.loading ? options.fn(this) : options.inverse(this);
    });
    
    // Date helpers
    Handlebars.registerHelper('year', () => new Date().getFullYear());
    
    // Default value helper (pipe syntax replacement)
  ...
  async loadModuleTemplates(modules) {
    for (const module of modules) {
      ...
      // Find all files (both .template and regular files)
      const templatePattern = join(templatePath, '**/*.template');
      const allFilesPattern = join(templatePath, '**/*');
      
      // First try to find .template files (including dotfiles)
      let templateFiles = await glob(templatePattern, { dot: true });
      
      // If no .template files found, use all files
      if (templateFiles.length === 0) {
        const allFiles = await glob(allFilesPattern, { nodir: true, dot: true });
        templateFiles = allFiles.filter(f => {
          // Exclude common non-template files
