...
export class ModuleRegistry {
  constructor() {
    ...
    this.implementationsPath = join(__dirname, 'implementations');
  }
  ...
  async discover() {
    ...
    try {
      ...
      // Load implementation modules first
      await this.loadImplementationModules();
      
      // Then scan module directories
   ...
   */
  async loadImplementationModules() {
    if (!existsSync(this.implementationsPath)) {
      return;
    }

    const files = readdirSync(this.implementationsPath)
      .filter(file => file.endsWith('-module.js'));

    for (const file of files) {
      try {
        const modulePath = join(this.implementationsPath, file);
        ...
        if (ModuleClass && typeof ModuleClass === 'function') {
          ...
          this.categories.add(moduleInstance.category);
          
          console.log(chalk.gray(`  📦 Loaded implementation: ${moduleInstance.name} (${moduleInstance.category})`));
        }
      } catch (error) {
        console.warn(chalk.yellow(`  ⚠️  Failed to load implementation ${file}:`), error.message);
      }
    }
  }
    ...
    for (const itemName of items) {
      ...
      if (existsSync(moduleJsonPath)) {
        ...
        for (const moduleName of modules) {
          ...
          // Check if this directory actually contains a module
          const hasModuleJson = existsSync(join(modulePath, 'module.json'));
          const hasConfigJson = existsSync(join(modulePath, 'config.json'));
          
          if (hasModuleJson || hasConfigJson) {
