...
export class OnboardingOrchestrator {
  ...
  /**
   * Get ordered list of ALL registered steps (to be filtered during execution)
  ...
  async execute(initialContext = {}) {
    ...
    const allSteps = this.getExecutionOrder();

    // Filter steps that should run initially (will be re-evaluated for each step)
    const stepsToRun = allSteps.filter(step => step.shouldRun(this.context));
    console.log(chalk.blue(`\n🚀 Starting onboarding with ${stepsToRun.length} steps...\n`));

    for (const step of allSteps) {
      // Check if step should run with current context
      if (!step.shouldRun(this.context)) {
        continue;
      }
