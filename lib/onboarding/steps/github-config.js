...
export class GitHubConfigurationStep extends OnboardingStep {
  ...
  async setupGitHubLabels(targetDir) {
    ...
    try {
      ...
      console.log(chalk.gray('   Then run: fsd labels create'));
      return;
    }
    ...
    // Import labelsManager and run setup directly
    try {
      console.log(chalk.blue('\n🏷️  Setting up GitHub labels...'));
      const { labelsManager } = await import('../../labels-manager.js');
      
      // Use the original cwd since we need to check git remote from the target directory
      const originalCwd = process.cwd();
      process.chdir(targetDir);
      
      await labelsManager.setupLabels({
        collection: 'ai-enhanced',
        emoji: true,
        force: false
      });
      ...
      console.log(chalk.yellow('⚠️  Failed to set up labels:'));
      console.log(chalk.gray(`   ${error.message}`));
      console.log(chalk.gray('   You can manually run: fsd labels create'));
    }
  }
