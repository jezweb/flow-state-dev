...
async function findMemoryFiles() {
  ...
  if (fs.existsSync(claudeProjectsDir)) {
    try {
      ...
      for (const project of projects) {
        const stat = await fs.lstat(path.join(claudeProjectsDir, project));
        if (stat.isDirectory()) {
  ...
  try {
    const gitDirs = await findRecentGitRepos();
    for (const gitDir of gitDirs) {
      const gitMemory = path.join(gitDir, 'CLAUDE.md');
      if (fs.existsSync(gitMemory)) {
        locations.push({
          path: gitMemory,
          type: `Git repo: ${path.basename(gitDir)}`
        });
      }
    }
  } catch (error) {
    // Ignore errors finding git repos
  }
    ...
    if (fs.existsSync(location.path)) {
      ...
      if (location.path !== userMemoryPath) {
        ...
        try {
          ...
          if (content.includes('# User Memory') || content.includes('## Personal Information')) {
            const stats = await fs.stat(location.path);
            location.size = formatFileSize(stats.size);
            location.modified = stats.mtime.toLocaleDateString();
            files.push(location);
          }
        } catch (error) {
          // Skip invalid files
