...
const checks = [
  ...
  },
  {
    ...
    description: 'Check for local Supabase development tools',
    check: async () => {
      const results = {
        docker: false,
        supabase: false,
      ...
      // Determine status
      if (results.docker && results.supabase) {
        return {
          status: CHECK_PASS,
          message: 'Local development tools installed (Docker + Supabase CLI)'
        };
      } else if (results.docker || results.supabase) {
        const missing = [];
        if (!results.docker) missing.push('Docker');
        if (!results.supabase) missing.push('Supabase CLI');
        
        return {
          status: CHECK_WARN,
          message: `Missing local dev tools: ${missing.join(', ')}`,
          fix: 'Run "fsd setup-local" to install missing tools'
        };
      } else {
        return {
          status: CHECK_WARN,
          message: 'No local development tools found',
          fix: 'Run "fsd setup-local" for local Supabase development'
        };
      }
    }
  },
  {
    ...
    description: 'Check local Supabase configuration',
    check: async (projectPath) => {
      ...
      if (!await fs.pathExists(supabaseDir)) {
        return {
          status: CHECK_WARN,
          message: 'Supabase not initialized for local development',
          fix: 'Run "fsd supabase init" to set up local development'
        };
      }
      ...
      try {
        ...
        return {
          status: CHECK_WARN,
          message: 'Local Supabase initialized but not running',
          fix: 'Run "fsd supabase start" to start local development'
        };
      }
    }
  }
